...
6. Generate walls, doors/windows, and beams per floor, waiting for each floor result before calling the next floor. Avoid parallel per-floor calls because each call writes a new `designBasis` revision.
